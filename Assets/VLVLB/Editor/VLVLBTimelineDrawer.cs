    ...
    public class VLVLBTimelineDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var isLoadButton = objectReferenceValue.resolvedVlvlbClipProfile != null &&
                               !objectReferenceValue.useProfile;

            EditorGUI.BeginDisabledGroup(!isSave);
            if (GUI.Button(
                    new Rect(position.x + position.width - buttonWidth, position.y, buttonWidth,
                        EditorGUIUtility.singleLineHeight), "Save"))
            {
            ...
            if (GUI.Button(
                    new Rect(position.x + position.width - buttonWidth * 2 - buttonMargine, position.y, buttonWidth,
                        EditorGUIUtility.singleLineHeight), "Load"))
            {
            ...
            if (GUI.Button(
                    new Rect(position.x + position.width - buttonWidth * 3 - buttonMargine * 2, position.y, buttonWidth,
                        EditorGUIUtility.singleLineHeight), "Export"))
            {
            ...
            defaultProps = PropertyDrawerUtility.DrawDefaultGUI(position, property.FindPropertyRelative("props"),
                new GUIContent("props"));
            EditorGUI.EndDisabledGroup();
